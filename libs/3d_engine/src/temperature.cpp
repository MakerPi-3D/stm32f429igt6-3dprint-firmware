...
static void detect_extruder_thermistor_falls_off(const float &pid_input, const float &pid_error)
{
  ...
  if (detect_get_peroid < MILLIS())
  {
    detect_get_peroid = MILLIS() + 1000;
    detect_time_count++;
...
void cal_pid_output(const int16_t &extruder, const float &pid_input, float &pid_output)
{
  ...
  else if ((0 == t_sys_data_current.enable_v5_extruder || K5 == t_sys_data_current.model_id) && pid_error > PID_FUNCTIONAL_RANGE)
  {
    ...
    if (t_sys_data_current.enable_color_mixing && current_pid_output_factor[extruder] == temp_get_pid_output_max_factor(extruder))
    {
      pid_output = BANG_MAX;
    }
...
void updateTemperaturesFromRawValues()
{
  for (uint8_t e = 0; e < EXTRUDERS; e++)
  {
    ...
    if (12 == t_sys_data_current.custom_model_id || 9 == t_sys_data_current.custom_model_id)
      current_temperature[e] = current_temperature_raw[e] * 660.0f / 1024.0f / OVERSAMPLENR;
    else
...
void temperature_init(bool _is_open_cavity)
{
  temp_extruders_init();
  #ifdef BED_MAXTEMP
  ...
  is_open_cavity = _is_open_cavity;
}
...
void temp_status_switch(unsigned long *raw_temp_0_value, unsigned long &raw_temp_bed_value, unsigned long &raw_temp_cavity_value, unsigned char &temp_count)
{
  ...
  case 1: // Measure TEMP_0
    raw_temp_0_value[0] += temperature_adc_get_value(0);
    temp_state = 2;
    break;
  ...
  case 6: // Prepare TEMP_2
    temperature_adc_status_switch(3);
    temp_state = 7;
    break;

  case 7: // Measure TEMP_2
    raw_temp_0_value[1] += temperature_adc_get_value(3);
    temp_state = 8;
    break;
