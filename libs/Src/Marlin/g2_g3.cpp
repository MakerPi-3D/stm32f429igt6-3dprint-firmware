...
namespace gcode
{
  ...
  static void prepare_arc_move(const volatile uint8_t isclockwise)
  {
    ...
    if (flash_param_t.extruder_type == EXTRUDER_TYPE_LASER)
    {
      arc_target[X_AXIS] = ccm_param::motion_3d_model.xyz_home_pos[X_AXIS];
    ...
    {
      arc_target[X2_AXIS] = ccm_param::motion_3d_model.xyz_home_pos[X2_AXIS];
    ...
    for (uint8_t i = 0U; i < ccm_param::motion_3d.axis_num; ++i)
    {
