...
static long plan_position[MAX_NUM_AXIS];   //rescaled from extern when axis_steps_per_unit are changed by gcode
static long plan_target[MAX_NUM_AXIS];
...
static void planner_reverse_pass()
{
  ...
  if ((((ccm_param::block_buffer_head - tail) + BLOCK_BUFFER_SIZE) & (BLOCK_BUFFER_SIZE - 1)) > 3)
  {
    ...
    while (block_index != tail)
    {
      ...
      block[2] = block[1];
      block[1] = block[0];
      block[0] = &ccm_param::block_buffer[block_index];
...
static void planner_forward_pass()
{
  ...
  while (block_index != ccm_param::block_buffer_head)
  {
    block[0] = block[1];
    block[1] = block[2];
    block[2] = &ccm_param::block_buffer[block_index];
...
static void planner_recalculate_trapezoids()
{
  ...
  {
    current = next;
    next = &ccm_param::block_buffer[block_index];
...
static void plan_prevent_dangerous_extrude(const long target, long &position, const int axis, const float extruder_temp)
{
  if (target != position)
  {
    if (extruder_temp < ccm_param::motion_3d.extrude_min_temp)
    {
    ...
    if (labs(target - position) > planner_settings.axis_steps_per_mm[axis]*ccm_param::motion_3d_model.extrude_maxlength)
    {
      position = target;  //behave as if the move really took place, but ignore E part
    }
...
static void plan_init_target(float (&plan_buffer_position)[MAX_NUM_AXIS], const float extruder0_temp, const float extruder1_temp)
{
  ...
  {
    plan_target[i] = lroundf(plan_buffer_position[i] * planner_settings.axis_steps_per_mm[i]);
  }
...
void plan_calc_millimeters_idex(float (&delta_mm)[MAX_NUM_AXIS], volatile float &millimeters)
{
  ...
  millimeters = sqrt(square(delta_mm_x) + square(delta_mm[Y_AXIS]) + square(delta_mm_z));
}
...
static void plan_calc_millimeters(float (&delta_mm)[MAX_NUM_AXIS], block_t *block, volatile float &millimeters, const volatile int &extrudemultiply)
{
  for (int i = 0; i < ccm_param::motion_3d.axis_num; i++)
  {
    delta_mm[i] = (float)(plan_target[i] - plan_position[i]) / planner_settings.axis_steps_per_mm[i];
  ...
  {
    is_x2_leq_dropsegments = (block->steps_axis[X2_AXIS] <= dropsegments);
  }

  if (block->steps_axis[X_AXIS] <= dropsegments && block->steps_axis[Y_AXIS] <= dropsegments &&
      block->steps_axis[Z_AXIS] <= dropsegments && block->steps_axis[Z2_AXIS] <= dropsegments && is_x2_leq_dropsegments)
  {
    millimeters = fabs(delta_mm[E_AXIS]) + (t_sys_data_current.enable_color_mixing ? fabs(delta_mm[B_AXIS]) : 0);
  }
  else
  {
    if (flash_param_t.extruder_type == EXTRUDER_TYPE_DUAL && t_sys.is_idex_extruder == 1) // idex 双头
    {
...
{
  float safe_speed = planner_settings.max_xy_jerk / 2;

  if (fabs(current_speed[Z_AXIS]) > planner_settings.max_z_jerk / 2)
  {
    safe_speed = min(safe_speed, planner_settings.max_z_jerk / 2);
  }

  if (fabs(current_speed[Z2_AXIS]) > planner_settings.max_z_jerk / 2)
  {
    safe_speed = min(safe_speed, planner_settings.max_z_jerk / 2);
  }

  if (fabs(current_speed[E_AXIS]) > planner_settings.max_e_jerk / 2)
  {
    safe_speed = min(safe_speed, planner_settings.max_e_jerk / 2);
  }

  if (1 == t_sys_data_current.enable_color_mixing)
  {
    if (fabs(current_speed[B_AXIS]) > planner_settings.max_b_jerk / 2)
    {
      safe_speed = min(safe_speed, planner_settings.max_b_jerk / 2);
    }
  }
...
static void plan_calc_safe_speed(float (&current_speed)[MAX_NUM_AXIS], int moves_queued, float safe_speed, volatile float nominal_speed, volatile float &max_entry_speed)
{
  ...
  if ((moves_queued > 1) && (previous_nominal_speed > 0.0001f))
  {
    ...
    //    }
    if (jerk > planner_settings.max_xy_jerk)
    {
      vmax_junction_factor = (planner_settings.max_xy_jerk / jerk);
    }

    if (flash_param_t.extruder_type == EXTRUDER_TYPE_DUAL && t_sys.is_idex_extruder == 1) // idex 双头
    {
      float jerk2 = sqrt(pow((current_speed[X2_AXIS] - previous_speed[X2_AXIS]), 2) + pow((current_speed[Y_AXIS] - previous_speed[Y_AXIS]), 2));

      if (jerk2 > planner_settings.max_xy_jerk)
      {
        vmax_junction_factor = min(vmax_junction_factor, (planner_settings.max_xy_jerk / jerk2));
      }
    }

    if (fabs(current_speed[Z_AXIS] - previous_speed[Z_AXIS]) > planner_settings.max_z_jerk)
    {
      double ZSpeedDiffVal = fabs(current_speed[Z_AXIS] - previous_speed[Z_AXIS]);
      vmax_junction_factor = min(vmax_junction_factor, (planner_settings.max_z_jerk / ZSpeedDiffVal));
    }

    if (fabs(current_speed[Z2_AXIS] - previous_speed[Z2_AXIS]) > planner_settings.max_z_jerk)
    {
      double ZSpeedDiffVal = fabs(current_speed[Z2_AXIS] - previous_speed[Z2_AXIS]);
      vmax_junction_factor = min(vmax_junction_factor, (planner_settings.max_z_jerk / ZSpeedDiffVal));
    }

    if (fabs(current_speed[E_AXIS] - previous_speed[E_AXIS]) > planner_settings.max_e_jerk)
    {
      double ESpeedDiffVal = fabs(current_speed[E_AXIS] - previous_speed[E_AXIS]);
      vmax_junction_factor = min(vmax_junction_factor, (planner_settings.max_e_jerk / ESpeedDiffVal));
    }

    if (1 == t_sys_data_current.enable_color_mixing)
    {
      if (fabs(current_speed[B_AXIS] - previous_speed[B_AXIS]) > planner_settings.max_b_jerk)
      {
        double BSpeedDiffVal = fabs(current_speed[B_AXIS] - previous_speed[B_AXIS]);
        vmax_junction_factor = min(vmax_junction_factor, (planner_settings.max_b_jerk / BSpeedDiffVal));
      }
    }
...
static void plan_calc_acceleration(block_t *block)
{
  ...
  {
    block->acceleration_st = (unsigned long)ceil(planner_settings.retract_acceleration * steps_per_mm); // convert to: acceleration steps/sec^2
  }
  else
  {
    block->acceleration_st = (unsigned long)ceil(planner_settings.acceleration * steps_per_mm); // convert to: acceleration steps/sec^2

    // Limit acceleration per axis
    for (int i = 0; i < ccm_param::motion_3d.axis_num; i++)
    {
      if (((float)block->acceleration_st * (float)block->steps_axis[i] / (float)block->step_event_count) > axis_steps_per_sqr_second[i])
      {
...
static void plan_calc_nominal_speed_and_rate(block_t *block, float (&current_speed)[MAX_NUM_AXIS], float (&delta_mm)[MAX_NUM_AXIS], int moves_queued, float feed_rate)
{
  ...
  {
    if (segment_time < planner_settings.min_segment_time_us)
    {
      // buffer is draining, add extra time.  The amount of time added increases if the buffer is still emptied more.
      inverse_second = 1000000.0f / (segment_time + (unsigned long)lroundf(2.0f *
                                     (planner_settings.min_segment_time_us - segment_time) /
                                     (t_sys.is_planner_slow_down ? t_sys.serial_moves_queued : moves_queued)));
    }
  }
  ...
  for (int i = 0; i < ccm_param::motion_3d.axis_num; i++)
  {
    ...
    if (fabs(current_speed[i]) > planner_settings.max_feedrate_mm_s[i])
    {
      double fabs_current_speed = fabs(current_speed[i]);
      speed_factor = min(speed_factor, planner_settings.max_feedrate_mm_s[i] / fabs_current_speed);
    }
  }
  ...
  {
    for (unsigned char i = 0; i < ccm_param::motion_3d.axis_num; i++)
    {
...
static void plan_init_feed_rate(block_t *block, volatile float &feed_rate)
{
  ...
  {
    if (feed_rate < planner_settings.min_travel_feedrate_mm_s)
    {
      feed_rate = planner_settings.min_travel_feedrate_mm_s;
    }
    ...
    {
      if (feed_rate > max_travel_feed_rate)
      {
  ...
  {
    if (feed_rate < planner_settings.min_feedrate_mm_s)
    {
      feed_rate = planner_settings.min_feedrate_mm_s;
    }
  }
}
...
void planner_buffer_line(planner_running_status_t *running_status, bool is_sync_z)
{
  ...
  // 初始化目标位置
  plan_init_target(plan_buffer_position_tmp, running_status->extruder0_temp, running_status->extruder1_temp);
  ...
  memcpy(&ccm_param::runningStatus[ccm_param::block_buffer_head], running_status, sizeof(planner_running_status_t));
  // Prepare to set up new block
  block_t *block = &ccm_param::block_buffer[ccm_param::block_buffer_head];
...
void planner_set_position_basic(const volatile float *_current_position, bool is_sync_z)
{
  for (int i = 0; i < ccm_param::motion_3d.axis_num; i++)
  {
    ...
    plan_position[i] = lroundf(_current_position[i] * planner_settings.axis_steps_per_mm[i]);
  }
...
{
  plan_position[axis] = lroundf(value * planner_settings.axis_steps_per_mm[axis]);

  if (is_sync_z == true && axis == Z_AXIS)
  {
    plan_position[Z2_AXIS] = lroundf(value * planner_settings.axis_steps_per_mm[Z2_AXIS]);
  }
