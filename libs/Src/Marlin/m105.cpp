...
namespace gcode
{
  void m105_process(void)
  {
    ...
    {
      USER_EchoLogStr("ok T%d:%.1f /%.1f", cur_extruder, temperature_get_extruder_current((unsigned char)cur_extruder),
                      temperature_get_extruder_target((unsigned char)cur_extruder));
    }
    ...
    USER_EchoLogStr(" B:%.1f /%.1f", temperature_get_bed_current(), temperature_get_bed_target());
    //  #endif //TEMP_BED_PIN
    //腔体温度，先用热床温度代替20170906
    #if defined(TEMP_BED_PIN) && TEMP_BED_PIN_ON > -1

    if (t_sys.enable_cavity_temp)
    {
