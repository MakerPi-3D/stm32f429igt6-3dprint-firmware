...
{
  if (SetNozzleXValue > 3.0f)
    SetNozzleXValue = 3.0f;

  if (SetNozzleXValue < -3.0f)
    SetNozzleXValue = -3.0f;

  if (SetNozzleYValue > 3.0f)
    SetNozzleYValue = 3.0f;

  if (SetNozzleYValue < -3.0f)
    SetNozzleYValue = -3.0f;

  if (SetNozzleZValue > 3.0f)
    SetNozzleZValue = 3.0f;
...
{
  flash_param_t.dual_extruder_1_offset[0] = SetNozzleXValue;
  flash_param_t.dual_extruder_1_offset[1] = SetNozzleYValue;
  flash_param_t.dual_extruder_1_offset[2] = SetNozzleZValue;
...
static void RefreshTheInterface_NotM14_Left_dual(void)
{
  ...
  SetNozzleXValue = flash_param_t.dual_extruder_1_offset[0];
  SetNozzleYValue = flash_param_t.dual_extruder_1_offset[1];
  SetNozzleZValue = flash_param_t.dual_extruder_1_offset[2]; ///100.0f;   //转换成小数  0-200   0-2.0
  DisplayText_NotM14_Left_dual();
}

void dual_extruder_setting(void)
{
  if (gui::is_refresh())
  {
