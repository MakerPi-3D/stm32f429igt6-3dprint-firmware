...
static volatile bool axis_min_endstop[XYZ_NUM_AXIS] = {false};                     /*!< 最小限位状态 */
...
static volatile long endstops_trigsteps[XYZ_NUM_AXIS] = {0};                       /*!< 记录限位位置 */
...
static inline block_t *plan_get_current_block()
{
  ...
  block_t *block = &ccm_param::block_buffer[ccm_param::block_buffer_tail];
  block->busy = true;
  return (block);
}
  ...
  for (int i = 0; i < ccm_param::motion_3d.axis_num; i++)
  {
    ...
    if (counter[i] > 0)
    {
      ...
      //      else
      {
        ...
        if (counter[i] > 0 && PULSE_DELAY_US > 0 && i < XYZ_NUM_AXIS)
          stepper_delay_us(PULSE_DELAY_US);
...
static inline void set_dirs(void)
{
  for (int i = 0; i < ccm_param::motion_3d.axis_num; i++)
  {
    ...
    {
      // -direction
      stepper_axis_write_dir(i, true);
      count_direction[i] = -1;
    }
    else
    {
      // +direction
      stepper_axis_write_dir(i, false);
      count_direction[i] = 1;
    }
  }
}
...
static inline void set_dir_check_endstop_xyz(int axis)
{
  ...
  {
    // -direction
    bool min_endstop = stepper_axis_xyz_read_min(axis);

    if (ccm_param::motion_3d_model.xyz_home_dir[axis] == 1)
      min_endstop = false;
  ...
  {
    // +direction
    bool max_endstop = stepper_axis_xyz_read_max(axis);
...
{
  if (!axis_min_endstop[X_AXIS] || !axis_min_endstop[Y_AXIS] || !axis_min_endstop[Z_AXIS] || !axis_min_endstop[Z2_AXIS])
    return true;
...
void st_clear_xyz_min_endstops_hit(void)
{
  ...
  axis_min_endstop[Z2_AXIS] = false;
}
...
void st_process(void)
{
  ...
  if (st_current_block == NULL)
  {
    ...
    block_t *next_block = &ccm_param::block_buffer[(ccm_param::block_buffer_tail + 1) & (BLOCK_BUFFER_SIZE - 1)];
...
void st_set_position(const long *_current_position)
{
  TASK_ENTER_CRITICAL();
...
void st_set_position_axis(const long value, const int axis)
{
  TASK_ENTER_CRITICAL();
...
{
  return (float)endstops_trigsteps[axis] / planner_settings.axis_steps_per_mm[axis];
}

float st_get_position_length(int axis)
{
  return (float)count_position[axis] / planner_settings.axis_steps_per_mm[axis];
}
...
{
  if (t_sys_data_current.enable_powerOff_recovery && ccm_param::motion_3d.enable_poweroff_up_down_min_min)
  {
    // 门检测与断电上下限位冲突，开启门检测只能上下共限位
    if (!ccm_param::motion_3d.enable_check_door_open)
      return (stepper_axis_xyz_read_min(Z_AXIS) || stepper_axis_xyz_read_max(Z_AXIS)); // 新旧插法兼容：1、断电Zmax在Door引脚；2、断电Zmax在Zmax引脚
    else
      return stepper_axis_xyz_read_min(Z_AXIS);
  }
  else
  {
    return stepper_axis_xyz_read_max(Z_AXIS);
  }
}
