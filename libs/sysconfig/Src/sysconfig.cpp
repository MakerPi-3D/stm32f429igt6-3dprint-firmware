...
#define SYS_CONFIG_FILE                          ((char *)"1:/SysConfig.txt")
...
void SysConfigOperation::saveInfo(bool isFunction)
{
  ...
  sys_write_info_to_sd(SYS_CONFIG_FILE);                                         // 将修改后的  InfoBuf 数组数据写进SD卡
}
...
SysConfig::SysConfig()
{
  ...
  memset(t_sys_data_current.build_date_str, 0, sizeof(t_sys_data_current.build_date_str));                 // 初始化FWBuildDateBuf内存全为0 ( )
  memset(t_sys_data_current.boot_ver_str, 0, sizeof(t_sys_data_current.boot_ver_str));             // 初始化FWBootVersionBuf内存全为0 ( )
  memset(t_sys_data_current.app2_ver_str, 0, sizeof(t_sys_data_current.app2_ver_str));             // 初始化FWApp2VersionBuf内存全为0 ( )
  ...
  t_sys.lcd_type = LCD_TYPE_35_480320_SIZE;
...
{
  sys_read_info_from_sd(SYS_CONFIG_FILE);                                                  //从SD卡读取配置信息
...
void SysConfig::getFWBootVersion(void)
{
  ...
  if (VersionInfoHeadPtr != NULL)                                    //判断是否有匹配数据
  {
    ...
    (void)strncpy(t_sys_data_current.boot_ver_str, VersionInfoHeadPtr, (unsigned int)length); //将 Boorloader版本 信息的编号复制出来
    t_sys_data_current.boot_ver_str[length] = 0;
    char *Ptr = strstr((char *)t_sys_data_current.boot_ver_str, " ");               //补0 ，表示字符串结束
...
void SysConfig::getFWApp2Version(void)
{
  ...
  if (VersionInfoHeadPtr != NULL)                                    //判断是否有匹配数据
  {
    ...
    (void)strncpy(t_sys_data_current.app2_ver_str, VersionInfoHeadPtr, (unsigned int)length);  //将当前版本信息编号复制出来
    t_sys_data_current.app2_ver_str[length] = 0;
  }
  else
  {
    /*********没有数据匹配的情况*********/
...
void SysConfig::getGetFWBuildDate(void)
{
  ...
  {
    /***********有匹配数据情况************/
    (void)strncpy(t_sys_data_current.build_date_str, DateInfoPtr, 8);                   //将固件建立日期时间复制出来                                         //判断是否有匹配数据
    t_sys_data_current.build_date_str[8] = 0; //20151110                                // 字符串结束字符
  }
  else
  {
    /*********没有数据匹配的情况*********/
...
void SysConfig::getlogoID(void)
{
  ...
  if (LOGOID != NULL)
    t_sys_data_current.logo_id = (unsigned char)(*LOGOID - '0');  //计算保存是否开启开机LOGO界面
  else
  {
...
void SysConfig::getStatusInfoStr(void)
{
  ...
  if (BED_LEVEL_PRESSURE_SENSOR == t_sys_data_current.enable_bed_level)
    (void)strcat(t_sys.function_str, "+BL3");
  ...
  /******************版本信息******************/
  (void)snprintf(t_sys.version_str, sizeof(t_sys.version_str), "D%s-%s-%s", t_sys_data_current.build_date_str, t_sys_data_current.app2_ver_str, t_sys_data_current.boot_ver_str); //D20151110-V3.0.0-V2.0
}
