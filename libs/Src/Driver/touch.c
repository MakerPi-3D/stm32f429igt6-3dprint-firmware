...
u8 TP_Scan(u8 tp)
{
  ...
  {
    if (tp)(void)TP_Read_XY2(&tp_dev.x[0], &tp_dev.y[0]); //读取物理坐标
...
u8 TP_Get_Adjdata(void)
{
  ...
  if ((tempfac & 0xff) == 0X0A) //触摸屏已经校准过了
  {
    ...
    if (1 == is_touch_capacitive) // 电容屏返回
      return 1;
  ...
  while (1) //如果连续10分钟没有按下,则自动退出
  {
    ...
    if ((tp_dev.sta & 0xc0) == TP_CATH_PRES) //按键按下了一次(此时按键松开了.)
    {
      ...
      case 4:  //全部四个点已经得到
        ...
        tem1 *= tem1;
        tem2 *= tem2;
        d1 = (unsigned short)sqrt((float)(tem1 + tem2)); //得到1,2的距离
        tem1 = abs(pos_temp[2][0] - pos_temp[3][0]); //x3-x4
        tem2 = abs(pos_temp[2][1] - pos_temp[3][1]); //y3-y4
        tem1 *= tem1;
        tem2 *= tem2;
        d2 = (unsigned short)sqrt((float)(tem1 + tem2)); //得到3,4的距离
        fac = (float)d1 / d2;
        ...
        tem1 *= tem1;
        tem2 *= tem2;
        d1 = (unsigned short)sqrt((float)(tem1 + tem2)); //得到1,3的距离
        tem1 = abs(pos_temp[1][0] - pos_temp[3][0]); //x2-x4
        tem2 = abs(pos_temp[1][1] - pos_temp[3][1]); //y2-y4
        tem1 *= tem1;
        tem2 *= tem2;
        d2 = (unsigned short)sqrt((float)(tem1 + tem2)); //得到2,4的距离
        fac = (float)d1 / d2;
        ...
        tem1 *= tem1;
        tem2 *= tem2;
        d1 = (unsigned short)sqrt((float)(tem1 + tem2)); //得到1,4的距离
        tem1 = abs(pos_temp[0][0] - pos_temp[3][0]); //x2-x4
        tem2 = abs(pos_temp[0][1] - pos_temp[3][1]); //y2-y4
        tem1 *= tem1;
        tem2 *= tem2;
        d2 = (unsigned short)sqrt((float)(tem1 + tem2)); //得到2,3的距离
        fac = (float)d1 / d2;
